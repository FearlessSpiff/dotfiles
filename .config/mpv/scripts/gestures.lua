...
local function if_enabled(enable)
...
local function drag_handler(onDrag, onStart, onEnd, options)
	...
	if options.deadzone == nil then
		options.deadzone = false
	end
	if options.deadzone_reset == nil then
		options.deadzone_reset = "auto"
	end
	if options.tick_ms == nil then
		options.tick_ms = 48
	end
	if options.button == nil then
		options.button = "MOUSE_BTN0"
	end
	if options.input_delay == nil then
		options.input_delay = mp.get_property_native("input-doubleclick-time")
	end
	...
	local function reset()
		...
		ticker:kill()
		if options.deadzone_reset ~= "manual" then
	...
	ticker:kill() -- don't run unless initiated
	...
	return {
		...
		set_haste = function(h)
			haste = h
		end,
		reset_deadzone = function()
			active_deadzone = false
		end,
	}
end
...
local function seek_n_volume()
	...
	local inertia
	inertia = mp.add_periodic_timer(OPTS.inertia_tick_ms / 1000, function()
